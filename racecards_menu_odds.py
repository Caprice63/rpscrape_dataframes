...
def sub_menu_1(): #Scrape Race Cards
    os.system('cls')
...
def sub_menu_3(): #Scrape Race Results
    os.system('cls')
...
def sub_menu_4(): #Non Runners, Aids etc.
    os.system('cls')
    print('\t\t1) Non Runners\n')
    print('\t\t2) Travellers\n')
    print('\t\t3) Aids\n')
...
def getodds():
    ...
    if date_ext == '':
        df_odd = pd.DataFrame(get_rtv_odds())

    else:
        date_ext == 'tomorrow'
        df_odd = pd.DataFrame(get_rtv_odds('tomorrow'))
    
  #  df_odd.set_index('name')
...
def get_rtv_odds(ext = ''):
    ...
    #df_race['name'] = clean_name('name')

    df_race['name'] = df_race['name'].str.title()
    
    df_race['name'] = df_race['name'].str.replace("'",'')
    
    #df_race['name'].strip().replace("'", '').lower().title()
    ...
    save_excel(df, main_word=r'C:\Users\chris\Documents\UKHR\Python\Racecards\RacecardsOdds', date=date)
...
def get_rtv_data(url_ext, date, racetime):
    ...
    final_list = []
    resp = requests.get(url,headers=headersrtv)
...
#   if sys.argv[1].lower() == 'tomorrow':
    ...
    print('\t\flat_fname\n\n')
    dir_name = r'C:\Users\chris\rpscrape\racecards'
    savepath = (r'C:\Users\chris\Documents\UKHR\Python\Racecards')
    

    json_fname = os.path.join(dir_name, date+'.json')
    flat_fname = os.path.join(savepath, 'RacecardsCap json'+date+'.xlsx')
...
# Open the JSON file
    with open(json_fname, 'r', encoding='utf-8') as f:  # add encoding parameter
        data = json.load(f)

    # Flatten the data
    rows = []
    for region, region_data in data.items():
        for course, course_data in region_data.items():
            for off_time, off_time_data in course_data.items():
                row = off_time_data.copy()
                row['region'] = region
                row['course'] = course
                row['off_time'] = off_time
                for runner in row['runners']:
                    runner_row = row.copy()
                    runner_row.update(runner)
                    # Extract medical and trainer_14_days data
                    medical_data = runner.get('medical', [{}])[0]
                    runner_row['medical_date'] = medical_data.get('date', '')
                    runner_row['medical_type'] = medical_data.get('type', '')
                    trainer_14_days_data = runner.get('trainer_14_days', {})
                    runner_row['trainer_14_days_runs'] = trainer_14_days_data.get('runs', '')
                    runner_row['trainer_14_days_wins'] = trainer_14_days_data.get('wins', '')
                    runner_row['trainer_14_days_percent'] = trainer_14_days_data.get('percent', '')
                    
                    # Remove unwanted columns
                    del runner_row['runners']
                    del runner_row['trainer_14_days']
                    del runner_row['prev_trainers']
                    del runner_row['prev_owners']
                    del runner_row['quotes']
                    del runner_row['stable_tour']
    ...
    df_race = df_race.drop(['medical'], axis=1)

  #  df_race.to_excel(flat_fname, index=False)
  ...
  #  df_odd = getodds()
    ...
    df_race = df_race.merge(df_odd, on = ['name'], how='left')

    # function to split jockey column into jockey_name and allowance
    df_race[['jockey_name', 'allow']] = df_race['jockey'].str.split("(", n=1, expand=True)
    df_race['allow'] = df_race['allow'].str.replace(')', '')

    # drop original jockey column
    df_race = df_race.drop('jockey', axis=1)

    df_race = df_race.rename({'jockey_name': 'jockey'}, axis=1) 
